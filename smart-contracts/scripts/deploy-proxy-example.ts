...
async function run() {
  ...
  const tkoControllerContractFactory = await getContractFactory("controller", signer);
  const tkoProxyContractFactory = await getContractFactory("proxy", signer);
  ...
  const tkoControllerContract = await tkoControllerContractFactory.deploy("new", "93214", {
    gasLimit: "400000000000",
    value: "1000 UNIT",
  });

  const tkoProxyContract = await tkoProxyContractFactory.deploy(
      "new",
      tkoControllerContract.address.toString()
  );

  console.log("");
  console.log(
    "Deploy successfully. The contracts addresses: ",
    `Controller: ${tkoControllerContract.address.toString()}`,
    `Proxy: ${tkoProxyContract.address.toString()}`,
  );

  const result = await tkoProxyContract.query.get_fees();

  console.log("get_fees via proxy", result);
